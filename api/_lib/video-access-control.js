...
exports.checkVideoAccess = checkVideoAccess;
exports.videoAccessMiddleware = videoAccessMiddleware;
const database_1 = require("./database");
const User_1 = __importDefault(require("../../src/models/User.js"));
const Video_1 = __importDefault(require("../../src/models/Video.js"));
/**
 * Check if user has access to a specific video
 * Requires either wallet connection (credits > 0) OR video unlock
 */
async function checkVideoAccess(req) {
    try {
        ...
        let user;
        if (req.userId) {
            user = await User_1.default.findById(req.userId);
        }
        else if (req.walletAddress) {
            user = await User_1.default.findOne({ wallet_address: req.walletAddress.toLowerCase() });
        }
        if (!user) {
            return {
                allowed: false,
 ...
 */
async function videoAccessMiddleware(req, res, next) {
    try {
