...
async function getVideoDetails(db, videoId, res) {
    try {
...
async function deductCredit(db, videoId, req, res) {
    try {
        ...
        // Deduct credit
        await db.collection('users').updateOne({ walletAddress: walletAddress.toLowerCase() }, { $inc: { credits: -1 } });
        // Record unlock
...
async function processTip(db, videoId, req, res) {
    try {
        ...
        const tipAmount = 0.1;
        // Deduct from tipper
        await db.collection('users').updateOne({ walletAddress: fromWallet.toLowerCase() }, { $inc: { credits: -tipAmount } });
        // Add to creator
        await db.collection('users').updateOne({ walletAddress: video.creatorWallet.toLowerCase() }, { $inc: { tipBalance: tipAmount, totalTipsReceived: tipAmount } });
        // Record tip
        await db.collection('tips').insertOne({
            videoId: new mongodb_1.ObjectId(videoId),
...
async function deleteVideo(db, videoId, req, res) {
    try {
        const { walletAddress } = req.body;
        if (!walletAddress) {
            return res.status(400).json({ error: 'Wallet address is required' });
        ...
        const video = await db.collection('videos').findOne({
            _id: new mongodb_1.ObjectId(videoId)
        });
        if (!video) {
            return res.status(404).json({ error: 'Video not found' });
        }
...
async function uploadVideo(db, videoId, req, res) {
    try {
        ...
        if (videoId === 'new') {
            ...
            const result = await db.collection('videos').updateOne({ _id: new ObjectId(videoId) }, { $set: videoData });
