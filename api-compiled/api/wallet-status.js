...
const wallet_verification_js_1 = require("./_lib/wallet-verification.js");
async function handler(req, res) {
    ...
    try {
        ...
        if (wallet && typeof wallet === 'string') {
            const verification = await (0, wallet_verification_js_1.verifyWalletConnection)(wallet);
        ...
        if (userId && typeof userId === 'string') {
            const status = await (0, wallet_verification_js_1.getWalletConnectionStatus)(userId);
            return res.status(200).json({
                success: true,
