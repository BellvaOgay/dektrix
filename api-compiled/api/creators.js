...
async function handleGet(req, res) {
    try {
        ...
        // Get top earners
        if (top_earners === 'true') {
            const creators = await Creator.getTopEarners(parseInt(limit));
            return res.status(200).json({ creators });
        }
        // Get specific creator by wallet address
        if (wallet_address) {
            const creator = await Creator.findByWallet(wallet_address)
        ...
        if (username) {
            const creator = await Creator.findOne({ username: username })
                .populate('uploadedVideos', 'title views createdAt');
            if (!creator) {
                return res.status(404).json({ error: 'Creator not found' });
            }
...
async function handlePut(req, res) {
    try {
        ...
        const creator = await Creator.findOneAndUpdate({ wallet_address: wallet_address.toLowerCase() }, { $set: updateData }, { new: true, runValidators: true });
        if (!creator) {
            return res.status(404).json({ error: 'Creator not found' });
        }
...
async function handleDelete(req, res) {
    try {
        ...
        const creator = await Creator.findOneAndUpdate({ wallet_address: wallet_address.toLowerCase() }, { $set: { isActive: false } }, { new: true });
        if (!creator) {
            return res.status(404).json({ error: 'Creator not found' });
        }
